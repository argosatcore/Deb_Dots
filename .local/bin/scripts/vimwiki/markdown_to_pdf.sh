...
pdftoread="$(echo "${doc%%.*}" | cut -d/ -f2)"
	if [ -n "$doc" ]; then
	pandoc "$doc" --pdf-engine=xelatex -V 'fontsize:12pt' --variable monofont="Menlo" -V "geometry:margin=2.54cm" -V 'papersize:letter' -M lang:es -s -o $HOME/Desktop/vimwiki/Pdfs/"$pdftoread".pdf \
