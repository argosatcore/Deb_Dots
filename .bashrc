...
# ------Handy aliases:
	...
	alias debcrawler='apt-cache seapt-cache pkgnames | fzf --multi --cycle --reverse --preview "apt-cache show {1}" --preview-window=:57%:wrap:rounded:hidden --bind=space:toggle-preview | xargs -o sudo apt install arch "" | sort | cut --delimiter " " --fields 1 | fzf --multi --cycle --reverse --preview "apt-cache show {1}" --preview-window=:57%:wrap:rounded:hidden --bind=space:toggle-preview | xargs -r sudo apt install -y'
